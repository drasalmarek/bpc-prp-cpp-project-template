    ...
    void node_pathfinder::subscriber_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        switch(state)
        {
            ...
            case State::SCANNING:
            {
                ...
                // publish the wanted angle
                angle_ += angle_to_gap_centre * 180.0 / M_PI; // convert to degrees
                state = State::TURNING;
                
                RCLCPP_INFO(this->get_logger(), "Angle to gap centre: %.2f degrees, angle_: %.2f", angle_to_gap_centre * 180.0 / M_PI, angle_);
            }
            case State::TURNING:
            {
                ...
                // publish the wanted speed
                message.data = 10.0f; // Example speed value
